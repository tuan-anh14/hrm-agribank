...
interface FetchDepartmentsParams {
    ...
    pageSize: number;
    search: string;
    sortBy: keyof Department | "createdAt";
    sortOrder: SortOrder;
    signal?: AbortSignal;
}

async function fetchDepartments(params: FetchDepartmentsParams): Promise<DepartmentsResponse> {
    const { page, pageSize, search, sortBy, sortOrder } = params;
    
    try {
        const raw = await getAllDepartmentsAPI();
...
interface UseDepartmentsReturn {
    ...
    actions: {
        ...
        toggleSort: (key: keyof Department | "createdAt") => void;
        reload: () => void;
    };
}

function useDepartments(initialPageSize: number = 10): UseDepartmentsReturn {
    const [page, setPage] = useState(1);
    const [pageSize, setPageSize] = useState(initialPageSize);
    const [search, setSearch] = useState("");
    const [sortBy, setSortBy] = useState<keyof Department | "createdAt">("createdAt");
    ...
    const toggleSort = useCallback((key: keyof Department | "createdAt") => {
        setPage(1);
        setSortBy((prevKey) => {
            if (prevKey === key) {
...
const ListDepartmentPage: React.FC = () => {
    const { state, actions } = useDepartments(10);
    ...
    const onTableChange = (
        ...
        if (s && s.order && s.field) {
            const key = String(s.field) as keyof Department | "createdAt";
            if (key !== sortBy) {
                ...
                render: (_, record) => (
                    <Space size="small">
                        ...
                        <Button
                            type="link"
                            icon={<EditOutlined />}
                            onClick={() => navigate(`/department/${record.id}/edit`)}
                            size="small"
                            style={{ color: '#faad14' }}
                        >
                            Sửa
                        ...
                        <Popconfirm
                            title="Xóa phòng ban"
                            description="Bạn có chắc chắn muốn xóa phòng ban này? Không thể xóa nếu còn nhân viên trong phòng ban."
    ...
    return (
        <Space direction="vertical" size={16} style={{ width: "100%" }}>
            ...
            {error && (
                <Alert type="error" showIcon message="Lỗi khi tải danh sách phòng ban" description={error} />
            )}
            ...
            {!loading && !error && data.length === 0 && (
                <Text type="secondary">Không tìm thấy phòng ban nào</Text>
            )}
        </Space>
    );
};
